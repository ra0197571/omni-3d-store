import React, { useState, useContext, useEffect } from 'react'; // 1. useEffect add kiya
import { db } from '../services/firebase';
import { collection, addDoc, doc, updateDoc, getDoc } from 'firebase/firestore'; // 2. getDoc add kiya
        ...
        <div className="flex-[1.5] space-y-6">
          <motion.div initial={{opacity:0, x:-20}} animate={{opacity:1, x:0}} className="bg-white p-10 rounded-[3rem] shadow-xl border border-slate-100">
            ...
            <form onSubmit={handleFinalOrder} className="space-y-5">
              {/* Value add kar di takay auto-fill nazar aaye */}
              <input type="text" placeholder="Full Name" value={custName} className="w-full p-5 bg-slate-50 rounded-2xl font-bold outline-none border focus:border-primary" onChange={e => setCustName(e.target.value)} />
              <textarea placeholder="Full Delivery Address" value={custAddress} className="w-full p-5 bg-slate-50 rounded-2xl h-28 font-bold outline-none border focus:border-primary" onChange={e => setCustAddress(e.target.value)} />
              ...
              <div className="pt-6 border-t">
                 ...
                 <div className="bg-slate-900 text-white p-6 rounded-[2.5rem] flex flex-col md:flex-row justify-between items-center gap-4">
                    {settings.jazzCashNo && <div className="text-center md:text-left"><p className="text-[10px] opacity-40">JAZZCASH</p><p className="font-black" style={{ color: themeColor }}>{settings.jazzCashNo}</p></div>}
                    {settings.easyPaisaNo && <div className="text-center md:text-left"><p className="text-[10px] opacity-40">EASYPAISA</p><p className="font-black" style={{ color: themeColor }}>{settings.easyPaisaNo}</p></div>}
                 </div>
